...
if arg.v == 0:
    util.rmdir(output_input_dir); util.mkdir(output_input_dir)
    util.rmdir(output_gt_dir); util.mkdir(output_gt_dir)
...
def process_batch(states, batch_id):
    ...
    scales = torch.rand(T, num_objects).mul_(2).add_(-1).mul_(scale_var).add_(1) # [1 - var, 1 + var]
    ratios = torch.rand(T, num_objects).mul_(2).add_(-1).mul_(ratio_var).add_(1).sqrt_() # [sqrt(1 - var), sqrt(1 + var)]
    for t in range(0, T):
...
util.save_json(data_config, path.join(output_dir, 'data_config.json'))
